...
void LI::calcRInts_ERI4D1(const int l, const int n_rints, const int n_rows, const int n_cols,
                          ...
                          double *rints_out)
{
    ...
    int n_hermite_cd = hermite_idxs_ket.size();
    int ofs0 = n_rints * 0;
    int ofs1 = n_rints * 1;
    int ofs2 = n_rints * 2;
    int ofs3 = n_rints * 3;
    ...
    for (int j = 0; j < n_hermite_cd; j++)
    {
        ...
        for (int i = 0; i < n_hermite_ab; i++)
        {
            ...
            int idx_tsp = icol * n_rows + irow;

            // d/dQ
            rints_out[ofs4 + idx_tsp] = sign_CD * fac * rints_3d(t + t_ + 1, u + u_, v + v_);
            rints_out[ofs5 + idx_tsp] = sign_CD * fac * rints_3d(t + t_, u + u_ + 1, v + v_);
            rints_out[ofs6 + idx_tsp] = sign_CD * fac * rints_3d(t + t_, u + u_, v + v_ + 1);

            // d/dS
            rints_out[ofs7 + idx_tsp] = sign_AB * fac * rints_3d(t + t_, u + u_, v + v_);
            ...
            // // d/dP^(cd)
            // rints_out[ofs4 + idx] = sign_CD * fac * rints_3d(t + t_ + 1, u + u_, v + v_);
            // rints_out[ofs5 + idx] = sign_CD * fac * rints_3d(t + t_, u + u_ + 1, v + v_);
            // rints_out[ofs6 + idx] = sign_CD * fac * rints_3d(t + t_, u + u_, v + v_ + 1);
        }
    }
}
