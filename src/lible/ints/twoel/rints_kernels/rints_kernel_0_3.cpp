...
template<>
void lible::ints::calcRInts_ERI_new<0, 3>(const double alpha, const double fac, const double *fnx,
                                          ...
                                          const int ofs_col, double *rints_out)
{
    ...
    rints_out[(ofs_row + 0) * n_cols + (ofs_col + 10)] = -1.0 * fac * rints_buff[13];
    rints_out[(ofs_row + 0) * n_cols + (ofs_col + 11)] = -1.0 * fac * rints_buff[14];
    rints_out[(ofs_row + 0) * n_cols + (ofs_col + 12)] = -1.0 * fac * rints_buff[15];
    rints_out[(ofs_row + 0) * n_cols + (ofs_col + 13)] = -1.0 * fac * rints_buff[16];
    rints_out[(ofs_row + 0) * n_cols + (ofs_col + 14)] = -1.0 * fac * rints_buff[17];
    rints_out[(ofs_row + 0) * n_cols + (ofs_col + 15)] = -1.0 * fac * rints_buff[18];
    rints_out[(ofs_row + 0) * n_cols + (ofs_col + 16)] = -1.0 * fac * rints_buff[19];
    rints_out[(ofs_row + 0) * n_cols + (ofs_col + 17)] = -1.0 * fac * rints_buff[20];
    rints_out[(ofs_row + 0) * n_cols + (ofs_col + 18)] = -1.0 * fac * rints_buff[21];
    rints_out[(ofs_row + 0) * n_cols + (ofs_col + 19)] = -1.0 * fac * rints_buff[22];
}
...
template void lible::ints::calcRInts_ERI2D1<0, 3>(const double alpha, const double fac, const double *fnx,
                                                  const double *xyz_ab, double *rints);

template void lible::ints::calcRInts_ERI2D2<0, 3>(const double alpha, const double fac, const double *fnx,
                                                  const double *xyz_ab, double *rints);

template void lible::ints::calcRInts_ERI3D1<0, 3>(const double alpha, const double fac, const double *fnx,
                                                  const double *xyz_pc, double *rints);

template void lible::ints::calcRInts_ERI4D1<0, 3>(const double alpha, const double fac, const double *fnx,
