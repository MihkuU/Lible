...
template lible::vec4d lible::ints::eri4KernelFun<1, 0, 1, 0>(const int ipair_ab, const int ipair_cd,
                                                             const ShellPairData &sp_data_ab,
                                                             const ShellPairData &sp_data_cd,
                                                             const ERI4Kernel *eri4_kernel);

template std::array<lible::vec4d, 12> lible::ints::eri4d1KernelFun<1, 0, 1, 0>(const int ipair_ab, const int ipair_cd,
                                                                               const ShellPairData &sh_data_ab,
                                                                               const ShellPairData &sp_data_cd,
                                                                               const ERI4D1Kernel *eri4d1_kernel);

template std::array<lible::vec4d, 12> lible::ints::eri4d1KernelFun<0, 1, 0, 1>(const int ipair_ab, const int ipair_cd,
                                                                               const ShellPairData &sh_data_ab,
                                                                               const ShellPairData &sp_data_cd,
                                                                               const ERI4D1Kernel *eri4d1_kernel);

template std::array<lible::vec4d, 12> lible::ints::eri4d1KernelFun<0, 1, 1, 0>(const int ipair_ab, const int ipair_cd,
                                                                               const ShellPairData &sh_data_ab,
                                                                               const ShellPairData &sp_data_cd,
                                                                               const ERI4D1Kernel *eri4d1_kernel);

template std::array<lible::vec4d, 12> lible::ints::eri4d1KernelFun<1, 0, 0, 1>(const int ipair_ab, const int ipair_cd,
                                                                               const ShellPairData &sh_data_ab,
                                                                               const ShellPairData &sp_data_cd,
                                                                               const ERI4D1Kernel *eri4d1_kernel);

template<> lible::vec4d
lible::ints::two::eri4Kernel<1, 0, 1, 0>(const int ipair_ab, const int ipair_cd,
                                         ...
                                         const ShellPairData &sp_data_cd)
{
    ...
    constexpr int la = 1, lb = 0, lc = 1, ld = 0;
    ...
    for (int ia = 0, iab = 0; ia < cdepth_a; ia++)
        for (int ib = 0; ib < cdepth_b; ib++, iab++)
        {
            ...
            eri4_batch(1, 0, 0, 0) += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[3];
            eri4_batch(1, 0, 0, 0) += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[0];
            eri4_batch(1, 0, 1, 0) += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[4];
            eri4_batch(1, 0, 1, 0) += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[1];
            eri4_batch(1, 0, 2, 0) += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[5];
            eri4_batch(1, 0, 2, 0) += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[2];
            eri4_batch(2, 0, 0, 0) += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[0];
            eri4_batch(2, 0, 0, 0) += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[6];
            eri4_batch(2, 0, 1, 0) += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[1];
            eri4_batch(2, 0, 1, 0) += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[7];
            eri4_batch(2, 0, 2, 0) += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[2];
            eri4_batch(2, 0, 2, 0) += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[8];
        }
...
template lible::vec3d lible::ints::eri3KernelFun<1, 0, 1>(const int ipair_ab, const int ishell_c,
                                                          const ShellPairData &sp_data_ab,
                                                          const ShellData &sh_data_c,
                                                          const ERI3Kernel *eri3_kernel);

template std::array<lible::vec3d, 9> lible::ints::eri3d1KernelFun<1, 0, 1>(const int ipair_ab, const int ishell_c,
                                                                           const ShellPairData &sh_data_ab,
                                                                           const ShellData &sh_data_c,
                                                                           const ERI3D1Kernel *eri3d1_kernel);

template std::array<lible::vec3d, 9> lible::ints::eri3d1KernelFun<0, 1, 1>(const int ipair_ab, const int ishell_c,
                                                                           const ShellPairData &sh_data_ab,
                                                                           const ShellData &sh_data_c,
                                                                           const ERI3D1Kernel *eri3d1_kernel);

template<> lible::vec3d
lible::ints::two::eri3Kernel<1, 0, 1>(const int ipair_ab, const int ishell_c,
                                      const std::vector<double> &ecoeffs_ab,
                                      ...
                                      const ShellData &sh_data_c)
{
    ...
    for (int ia = 0, iab = 0; ia < cdepth_a; ia++)
        for (int ib = 0; ib < cdepth_b; ib++, iab++)
        {
            ...
            eri3_batch(1, 0, 0) += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[3];
            eri3_batch(1, 0, 0) += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[0];
            eri3_batch(1, 0, 1) += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[4];
            eri3_batch(1, 0, 1) += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[1];
            eri3_batch(1, 0, 2) += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[5];
            eri3_batch(1, 0, 2) += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[2];
            eri3_batch(2, 0, 0) += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[0];
            eri3_batch(2, 0, 0) += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[6];
            eri3_batch(2, 0, 1) += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[1];
            eri3_batch(2, 0, 1) += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[7];
            eri3_batch(2, 0, 2) += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[2];
            eri3_batch(2, 0, 2) += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[8];
        }
...
template lible::vec2d lible::ints::eri2KernelFun<1, 1>(const int ishell_a, const int ishell_b,
                                                       const ShellData &sh_data_a,
                                                       const ShellData &sh_data_b,
                                                       const ERI2Kernel *eri2_kernel);

template std::array<lible::vec2d, 6> lible::ints::eri2d1KernelFun<1, 1>(const int ishell_a, const int ishell_b,
                                                                        const ShellData &sh_data_a,
                                                                        const ShellData &sh_data_b,
                                                                        const ERI2D1Kernel *eri2d1_kernel);

template<> lible::vec2d
lible::ints::two::eri2Kernel<1, 1>(const int ishell_a, const int ishell_b,
                                   ...
                                   const ShellData &sh_data_a, const ShellData &sh_data_b)
{
    ...
    for (int ia = 0; ia < cdepth_a; ia++)
    {
        ...
        eri2_batch(1, 0) += p_ecoeffs_a[5] * p_rints_x_ecoeffs[3];
        eri2_batch(1, 0) += p_ecoeffs_a[4] * p_rints_x_ecoeffs[0];
        eri2_batch(1, 1) += p_ecoeffs_a[5] * p_rints_x_ecoeffs[4];
        eri2_batch(1, 1) += p_ecoeffs_a[4] * p_rints_x_ecoeffs[1];
        eri2_batch(1, 2) += p_ecoeffs_a[5] * p_rints_x_ecoeffs[5];
        eri2_batch(1, 2) += p_ecoeffs_a[4] * p_rints_x_ecoeffs[2];
        eri2_batch(2, 0) += p_ecoeffs_a[8] * p_rints_x_ecoeffs[0];
        eri2_batch(2, 0) += p_ecoeffs_a[10] * p_rints_x_ecoeffs[6];
        eri2_batch(2, 1) += p_ecoeffs_a[8] * p_rints_x_ecoeffs[1];
        eri2_batch(2, 1) += p_ecoeffs_a[10] * p_rints_x_ecoeffs[7];
        eri2_batch(2, 2) += p_ecoeffs_a[8] * p_rints_x_ecoeffs[2];
        eri2_batch(2, 2) += p_ecoeffs_a[10] * p_rints_x_ecoeffs[8];
    }
...
template std::array<lible::vec2d, 6> lible::ints::two::eri2d1Kernel<1, 1>(const int ishell_a, const int ishell_b,
                                                           const std::vector<double> &ecoeffs_a,
                                                           const std::vector<double> &ecoeffs_b_tsp,
                                                           const ShellData &sh_data_a,
                                                           const ShellData &sh_data_b);
