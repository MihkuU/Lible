...
template<> void lible::ints::two::eri4Kernel<1, 0, 1, 0>(const int cdepth_a, const int cdepth_b,
                                                         ...
                                                         double *eri4_batch)
{
    ...
    for (int ia = 0, iab = 0; ia < cdepth_a; ia++)
        for (int ib = 0; ib < cdepth_b; ib++, iab++)
        {
            ...
            eri4_batch[3] += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[3];
            eri4_batch[3] += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[0];
            eri4_batch[4] += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[4];
            eri4_batch[4] += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[1];
            eri4_batch[5] += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[5];
            eri4_batch[5] += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[2];
            eri4_batch[6] += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[0];
            eri4_batch[6] += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[6];
            eri4_batch[7] += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[1];
            eri4_batch[7] += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[7];
            eri4_batch[8] += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[2];
            eri4_batch[8] += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[8];
        }
}

template<> void lible::ints::two::eri3Kernel<1, 0, 1>(const int cdepth_a, const int cdepth_b, const int cdepth_c,
                                                      const double* exps_a, const double* exps_b,
                                                      ...
                                                      double* eri3_batch)
{
    ...
    for (int ia = 0, iab = 0; ia < cdepth_a; ia++)
        for (int ib = 0; ib < cdepth_b; ib++, iab++)
        {
            ...
            eri3_batch[3] += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[3];
            eri3_batch[3] += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[0];
            eri3_batch[4] += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[4];
            eri3_batch[4] += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[1];
            eri3_batch[5] += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[5];
            eri3_batch[5] += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[2];
            eri3_batch[6] += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[0];
            eri3_batch[6] += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[6];
            eri3_batch[7] += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[1];
            eri3_batch[7] += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[7];
            eri3_batch[8] += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[2];
            eri3_batch[8] += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[8];
        }
}

template<> void lible::ints::two::eri2Kernel<1, 1>(const int cdepth_a, const int cdepth_b,
                                                   const double* exps_a, const double* exps_b,
                                                   const double* coords_a, const double* coords_b,
                                                   ...
                                                   double* eri2_batch)
{
    ...
    for (int ia = 0; ia < cdepth_a; ia++)
    {
            ...
            eri2_batch[3] += p_ecoeffs_a[5] * p_rints_x_ecoeffs[3];
            eri2_batch[3] += p_ecoeffs_a[4] * p_rints_x_ecoeffs[0];
            eri2_batch[4] += p_ecoeffs_a[5] * p_rints_x_ecoeffs[4];
            eri2_batch[4] += p_ecoeffs_a[4] * p_rints_x_ecoeffs[1];
            eri2_batch[5] += p_ecoeffs_a[5] * p_rints_x_ecoeffs[5];
            eri2_batch[5] += p_ecoeffs_a[4] * p_rints_x_ecoeffs[2];
            eri2_batch[6] += p_ecoeffs_a[8] * p_rints_x_ecoeffs[0];
            eri2_batch[6] += p_ecoeffs_a[10] * p_rints_x_ecoeffs[6];
            eri2_batch[7] += p_ecoeffs_a[8] * p_rints_x_ecoeffs[1];
            eri2_batch[7] += p_ecoeffs_a[10] * p_rints_x_ecoeffs[7];
            eri2_batch[8] += p_ecoeffs_a[8] * p_rints_x_ecoeffs[2];
            eri2_batch[8] += p_ecoeffs_a[10] * p_rints_x_ecoeffs[8];
    }
}
