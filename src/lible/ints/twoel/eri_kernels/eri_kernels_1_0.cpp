...
template<> void lible::ints::two::eri4Kernel<1, 0, 0, 0>(const int cdepth_a, const int cdepth_b,
                                                         ...
                                                         double *eri4_batch)
{
    ...
    for (int ia = 0, iab = 0; ia < cdepth_a; ia++)
        for (int ib = 0; ib < cdepth_b; ib++, iab++)
        {
            ...
            eri4_batch[1] += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[1];
            eri4_batch[1] += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[0];
            eri4_batch[2] += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[0];
            eri4_batch[2] += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[2];
        }
}

template<> void lible::ints::two::eri3Kernel<1, 0, 0>(const int cdepth_a, const int cdepth_b, const int cdepth_c,
                                                      const double* exps_a, const double* exps_b,
                                                      ...
                                                      double* eri3_batch)
{
    ...
    for (int ia = 0, iab = 0; ia < cdepth_a; ia++)
        for (int ib = 0; ib < cdepth_b; ib++, iab++)
        {
            ...
            eri3_batch[1] += p_ecoeffs_ab[5] * p_rints_x_ecoeffs[1];
            eri3_batch[1] += p_ecoeffs_ab[4] * p_rints_x_ecoeffs[0];
            eri3_batch[2] += p_ecoeffs_ab[8] * p_rints_x_ecoeffs[0];
            eri3_batch[2] += p_ecoeffs_ab[10] * p_rints_x_ecoeffs[2];
        }
}

template<> void lible::ints::two::eri2Kernel<1, 0>(const int cdepth_a, const int cdepth_b,
                                                   const double* exps_a, const double* exps_b,
                                                   const double* coords_a, const double* coords_b,
                                                   ...
                                                   double* eri2_batch)
{
    ...
    for (int ia = 0; ia < cdepth_a; ia++)
    {
            ...
            eri2_batch[1] += p_ecoeffs_a[5] * p_rints_x_ecoeffs[1];
            eri2_batch[1] += p_ecoeffs_a[4] * p_rints_x_ecoeffs[0];
            eri2_batch[2] += p_ecoeffs_a[8] * p_rints_x_ecoeffs[0];
            eri2_batch[2] += p_ecoeffs_a[10] * p_rints_x_ecoeffs[2];
    }
}
