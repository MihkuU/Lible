...
set<string> GCI::CIPSI::selectCFGsAndCSFs(wfn_ptr &wave_function)
{
    ...
    vector<string> prefixes_scattered = gci->prefix_algorithm->prefixBonanza(generators);
    n_prefixes = prefixes_scattered.size();

#ifdef _USE_MPI_
    // mpi::all_reduce(mpi::communicator(gci->world, mpi::comm_duplicate),
...
#pragma omp parallel
    {
        ...
        gci->prefix_algorithm->generateConfsAndConnections(prefixes_para[thread_num],
                                                           generators_by_roots,
                                                           wave_function,
                                                           data_fois_local,
                                                           data_var_local);
...
#pragma omp critical
     ...
     */
    appendSpinFunctions(onvs_sfs_reduced, gci->sfs_map__idx_to_sf,
                        gci->sfs_map__sf_to_idx);
     ...
     */
    gci->coupling_coeffs->constructCCs(sf_pairs_1el_cipsi, sf_pairs_2el_cipsi,
                                       gci->ccs_2el, gci->ccs_1el);
...
void GCI::CIPSI::appendCFGsAndCSFs(const map<string, vector<int>> &selected_cfgs_sfs,
                                   set<string> &selected_cfgs,
                                   wfn_ptr &wave_function)
{
    for (auto &item : selected_cfgs_sfs)
    {
        ...
        CFG cfg(spin, onv);
        int nue = cfg.getNUE();
        map<int, string> sfs_map = gci->sfs_map__idx_to_sf.at(nue);
...
GCI::CIPSI::generateGenerators(size_t &n_generators)
{    
    ...
    for (size_t iroot = 0; iroot < n_roots; iroot++)
    {
        ...
        dvec coeffs_iroot = arma::conv_to<arma::dvec>::from(gci->ci_vectors[iroot]);

        for (size_t iconf = 0; iconf < gci->wave_function->getNumCFGs(); iconf++)
        {
            size_t pos = gci->wave_function->getPos(iconf);
            size_t dim = gci->wave_function->getDim(iconf);
            string cfg = gci->wave_function->getONV(iconf);
...
void GCI::CIPSI::appendSpinFunctions(const map<string, set<string>> &onvs_sfs_reduced,
                                     ...
                                     map<int, map<string, int>> &sfs_map__sf_to_idx)
{
    ...
    for (auto &item : sfs_reduced)
    {
        ...
        gci->spin_functions[nue].insert(item.second.begin(), item.second.end());

        if (gci->sfs_map__sf_to_idx.find(nue) == gci->sfs_map__sf_to_idx.end())
        {
            size_t pos = 0;
            for (auto &sf : item.second)
            {
                gci->sfs_map__sf_to_idx[nue][sf] = pos;
                gci->sfs_map__idx_to_sf[nue][pos] = sf;
                pos++;
            }
        }
        else
        {
            for (auto &sf : item.second)
            {
                if (gci->sfs_map__sf_to_idx.at(nue).find(sf) == gci->sfs_map__sf_to_idx.at(nue).end())
                {
                    size_t pos = gci->sfs_map__sf_to_idx.at(nue).size();
                    gci->sfs_map__sf_to_idx[nue][sf] = pos;
                    gci->sfs_map__idx_to_sf[nue][pos] = sf;
                }
                else
...
vector<wfn_ptr> GCI::CIPSI::setUpCIPSIWaveFunction(const vector<DataFOIS> &data_fois_para)
{
    int num_threads;
#pragma omp parallel
    {
...
#pragma omp parallel
    {
        ...
        for (const auto &[icfg, sfs] : data_fois_para[thread_num].cfgs_sfs)
        {
            ...
            int nue = cfg->getNUE();
            map<string, int> sf_map = returnSFMap(gci->sfs_map__sf_to_idx.at(nue), sfs);
...
void GCI::CIPSI::constructSFPairs(const wfn_ptr &wave_function,
...
GCI::CIPSI::doCIPSIPruning(const vector<wfn_ptr> &wfn_cipsi_para,
                           const vector<DataFOIS> &data_fois_para)
{
...
#pragma omp parallel
    {
        ...
        dvec diag = arma::conv_to<dvec>::from(gci->calcDiag(wfn_cipsi_local));

        vector<dvec> sigma_vectors(n_roots);
        for (size_t iroot = 0; iroot < n_roots; iroot++)
            sigma_vectors[iroot] = arma::conv_to<dvec>::from(gci->calcSigma(data_fois_para[thread_num],
                                                                            gci->ci_vectors[iroot],
                                                                            wfn_cipsi_local));
        ...
        for (size_t iroot = 0; iroot < n_roots; iroot++)
        {
            ...
            energy_minus_diag.fill(gci->ci_energies[iroot]);
